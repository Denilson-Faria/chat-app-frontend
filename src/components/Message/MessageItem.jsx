...
const MessageItem = ({ message, isOwn, avatar, onReply, onMarkRead }) => {
  ...
  const getStickerUrl = () => {
    ...
    return `http://localhost:5000${message.stickerUrl}`;
  };
  ...
  return (
    ...
    >
      ...
      <div className={`flex flex-col max-w-[85%] sm:max-w-[75%] md:max-w-[70%] ${isOwn ? 'items-end' : 'items-start'}`}>
        ...
        {message.replyTo && (
          <div
            className={`mb-2 p-2 sm:p-2.5 rounded-xl text-xs sm:text-sm border-l-4 shadow-md backdrop-blur-sm transition-all duration-300 hover:shadow-lg hover:scale-[1.02] ${
              isOwn
        ...
        <div
          className={`relative group ${
            message.type === 'sticker' || message.type === 'image' || message.type === 'video'
              ? 'bg-transparent' 
              : `rounded-2xl sm:rounded-3xl px-3 sm:px-4 py-2 sm:py-2.5 shadow-lg transition-all duration-300 hover:shadow-xl hover:scale-[1.02] ${
                  isOwn
        ...
        >
          ...
          {message.type === 'image' && message.mediaData && (
            <div className="relative group">
              ...
              <img
                ...
                alt="Imagem enviada"
                className={`relative max-w-[280px] sm:max-w-[320px] max-h-[400px] object-contain rounded-2xl shadow-2xl transition-all duration-300 cursor-pointer hover:brightness-90 ${
              ...
              {message.text && message.text !== `📷 ${message.text.split('📷 ')[1]}` && (
                <p className={`mt-2 px-3 py-2 rounded-xl text-sm whitespace-pre-wrap break-words ${
                  isOwn 
              ...
              <button
                ...
                className={`absolute top-2 right-2 p-2 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-300 backdrop-blur-md shadow-lg hover:scale-110 ${
                  isOwn 
          ...
          {message.type === 'audio' && (
            ...
            >
              <div className={`p-2 sm:p-2.5 rounded-full shadow-lg transition-all duration-300 hover:scale-110 ${
                isOwn 
              ...
              <div className="flex-1">
                <div className={`h-1.5 rounded-full relative overflow-hidden ${
                  isOwn 
                    ? 'bg-blue-400/30' 
          ...
          <button
            onClick={handleReply}
            className={`absolute top-1/2 -translate-y-1/2 p-2 rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-300 shadow-lg backdrop-blur-sm hover:scale-110 active:scale-95 ${
              isOwn 
